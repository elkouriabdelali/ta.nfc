...
Persistent<FunctionTemplate> NfcTagProxy::proxyTemplate = Persistent<FunctionTemplate>();
jclass NfcTagProxy::javaClass = NULL;

NfcTagProxy::NfcTagProxy(jobject javaObject) : titanium::Proxy(javaObject)
{
}

void NfcTagProxy::bindProxy(Handle<Object> exports)
{
...
// Methods --------------------------------------------------------------------
Handle<Value> NfcTagProxy::getId(const Arguments& args)
{
	LOGD(TAG, "getId()");
	HandleScope scope;
	...
	if (!methodID) {
		methodID = env->GetMethodID(NfcTagProxy::javaClass, "getId", "()Ljava/lang/String;");
		if (!methodID) {
...
Handle<Value> NfcTagProxy::getTechList(const Arguments& args)
{
	LOGD(TAG, "getTechList()");
	HandleScope scope;
	...
	static jmethodID methodID = NULL;
	if (!methodID) {
		methodID = env->GetMethodID(NfcTagProxy::javaClass, "getTechList", "()[Ljava/lang/String;");
		if (!methodID) {
			const char *error = "Couldn't find proxy method 'getTechList' with signature '()[Ljava/lang/String;'";
			LOGE(TAG, error);
				return titanium::JSException::Error(error);
		}
	}
...
Handle<Value> NfcTagProxy::getter_id(Local<String> property, const AccessorInfo& info)
{
	LOGD(TAG, "get id");
	HandleScope scope;
...
Handle<Value> NfcTagProxy::getter_techList(Local<String> property, const AccessorInfo& info)
{
	LOGD(TAG, "get techList");
	HandleScope scope;
